...
class ValidationLCStrategy implements ValidationStrategyInterface
{
    public function valider(Facture $facture, $user): array
    {
        ...
        return ['success' => true, 'message' => 'Facture LC validée avec succès'];
    }
    ...
    public function rejeter(Facture $facture, $user, string $motif): array
    {
        ...
        if ($facture->fournisseur_id) {
            Notification::create([
                ...
                'type'              => 'FactureRejetee',
                'titre'             => 'Votre facture LC a été rejetée',
        ...
        return ['success' => true, 'message' => 'Facture LC rejetée avec succès'];
    }
            ...
            foreach ($agents as $agent) {
                if ($agent->isNotificationsEnabled) {
                    $client->post(env('NOTIFICATION_MAIL_URL'), [
                        'json' => [
                            'emails'  => [$agent->email],
                            'message' => 'Une facture LC est prête pour validation'
                        ]
                    ]);
                }
                Notification::create([
                    ...
                    'titre'             => 'Nouvelle facture LC à valider',
                    'num_facture'       => $facture->number,
                    'id_facture'        => $facture->id,
                    'id_reclamation'    => null,
                    'titre_reclamation' => null,
